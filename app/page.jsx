...
// CloudConfigModal（是否同步云端）已废弃：所有用户数据直接保存在后端 MySQL
...
import { authApi, storageApi, setToken, clearToken, parseJwtExp } from './api/backend';
...
export default function HomePage() {
  ...
  // 本地存储代理：
  // - 为满足“前端不落地业务数据”的要求，除 token 外，其他 key 均落到内存 Map。
  // - 业务数据由后端 user_storage 统一保存/同步。
  useEffect(() => {
  ...
  // 先保存“原始方法”
    const rawGetItem = real.getItem.bind(real);
    const rawSetItem = real.setItem.bind(real);
    const rawRemoveItem = real.removeItem.bind(real);
    const rawClear = real.clear.bind(real);

    const allowPersist = (k) => k === 'fund_token';

    const get = (k) => (allowPersist(k) ? rawGetItem(k) : (mem.has(k) ? mem.get(k) : null));
    const set = (k, v) => {
      if (allowPersist(k)) rawSetItem(k, v);
      else mem.set(k, String(v));
    };
    const remove = (k) => {
      if (allowPersist(k)) rawRemoveItem(k);
      else mem.delete(k);
    };
    const clear = () => {
      // 仅清理内存数据，token 由 clearToken 处理
      mem.clear();
      // 如果你希望 clear() 也清除真实 localStorage（一般不建议），才调用 rawClear()
      // rawClear();
    };

    // 只重写方法，避免替换对象导致浏览器报错
    try {
      real._fund_mem_getItem = real.getItem;
      real._fund_mem_setItem = real.setItem;
      real._fund_mem_removeItem = real.removeItem;
      real._fund_mem_clear = real.clear;
      real.getItem = get;
      real.setItem = set;
      real.removeItem = remove;
      real.clear = clear;
      real.key = (i) => Array.from(mem.keys())[i] || null;
      Object.defineProperty(real, 'length', { get: () => mem.size });
    } catch {
      // 若浏览器不允许覆写（极少数情况），则降级：仍然可用，但不保证不落地
    }
  }, []);
  const [userMenuOpen, setUserMenuOpen] = useState(false);
  const [loginModalOpen, setLoginModalOpen] = useState(false);
  const [loginMode, setLoginMode] = useState('login'); // login | register
  const [loginForm, setLoginForm] = useState({ username: '', password: '', nickname: '' });
  ...
  // 旧的“是否同步云端”弹窗已废弃（所有数据直接保存在后端 MySQL）
  ...
  useEffect(() => {
    // /api/auth/me 返回的是 { userId, username }
  ...
  const scheduleSync = useCallback(() => {
    ...
    syncDebounceRef.current = setTimeout(() => {
      // 为保证一致性：始终发送“全量快照”（后端会拆表落库/合并 patch）。
      // 这样即使前端某次只变更了一个 key，也不会因为覆盖导致其他字段丢失。
      const payload = collectLocalPayload(null);

      // 清空脏数据标记
      dirtyKeysRef.current.clear();

      // 比较是否真的变了（按归一化后的 payload）
  ...
  // 在页面隐藏/刷新/关闭时，尽量把最后的改动刷到后端，降低“刚改完就刷新”导致的丢失概率
  useEffect(() => {
    const flush = () => {
      if (!userIdRef.current) return;
      if (skipSyncRef.current) return;
  ...
  const loadUserAndStorage = async (isExplicitLogin = false) => {
    try {
      ...
      // 登录后拉取云端存储（user_storage），并覆盖到内存 localStorage/页面状态
      // 策略：按“最后写入覆盖”——即后端为准
      await fetchCloudConfig(me?.userId || me?.id);
    } catch (e) {
      // token 无效/过期
      clearToken();
      // 避免“上一个用户”的缓存残留（本项目会把业务数据保存到后端 MySQL）
  ...
  // 首次进入：若本地有 token，则尝试恢复会话，否则强制弹出登录
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const token = window.localStorage.getItem('fund_token') || window.sessionStorage.getItem('fund_token');
    if (!token) {
      setLoginModalOpen(true);
      return;
    }
    loadUserAndStorage(false);
  }, []);

  // token 续期：当 token 剩余 < 15 分钟时自动续期
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const token = window.localStorage.getItem('fund_token') || window.sessionStorage.getItem('fund_token');
    if (!token) return;
    ...
    if (!exp) return;
    const refreshAt = exp * 1000 - 15 * 60 * 1000;
    const delay = Math.max(1000, refreshAt - Date.now());
    const timer = setTimeout(async () => {
      try {
        const newToken = await authApi.refresh();
        if (isString(newToken) && newToken.length > 10) {
    ...
    try {
      if (loginMode === 'register') {
        await authApi.register({
          ...
          nickname: loginForm.nickname?.trim() || null,
        });
      }
  ...
  // 弱语义退出：前端清理 token 与本地状态，后端无需让 token 失效
  const handleLogout = async () => {
    isLoggingOutRef.current = true;
    try {
      clearToken();
      // 清空页面状态（内存 localStorage 也会被 clear）
      try { localStorage.clear(); } catch {}
      setUser(null);
      setUserMenuOpen(false);
      setLoginModalOpen(true);
      setLoginForm({ username: '', password: '', nickname: '' });
    } finally {
  ...
  const fetchCloudConfig = async (userId) => {
    // 旧逻辑：弹窗询问“是否同步云端”。
    // 新逻辑：服务端（MySQL）是唯一真源，登录后直接拉取并应用即可。
    if (!userId) return;
    try {
      const storage = await storageApi.get();
      const json = storage?.storageData || storage?.storage_data || '{}';
      const updatedAt = storage?.updatedAt || storage?.updated_at || null;
      let data = {};
      try {
      ...
      await applyCloudConfig(isPlainObject(data) ? data : {}, updatedAt);
    } catch (e) {
      console.error('获取服务端配置失败', e);
    }
  };

  const syncUserConfig = async (userId, showTip = false, payload = null, isPartial = false) => {
    if (!userId) {
    ...
    try {
      setIsSyncing(true);
      // 后端会以“增量 patch + 规范化表落库”的方式保存；
      // 为避免字段丢失，前端默认发送全量快照（payload 为空时会自动收集）。
      const dataToSync = payload || collectLocalPayload();
      const resp = await storageApi.save(JSON.stringify(dataToSync || {}));
      const updatedAt = resp?.updatedAt || resp?.updated_at || nowInTz().toISOString();
      storageHelper.setItem('localUpdatedAt', updatedAt);

      if (showTip) {
        setSuccessModal({ open: true, message: '已保存' });
      }
    } catch (e) {
  ...
  // handleSyncLocalConfig 已废弃：不再弹窗询问“是否同步”，改为登录后自动拉取/自动保存。
  ...
  return (
    <div className="container content">
      <div className="navbar glass" ref={navbarRef}>
        ...
        <div className={`brand ${(isSearchFocused || selectedFunds.length > 0) ? 'search-focused-sibling' : ''}`}>
          <div
            ...
            title={isSyncing ? '正在保存...' : undefined}
          >
            {/* 同步中图标 */}
            <svg
              width="20"
      ...
      {/* CloudConfigModal 已废弃：不再弹窗询问“是否同步云端” */}
